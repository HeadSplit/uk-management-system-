                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    @foreach($users as $user)
                        <tr>
                            ...
                            <td class="px-6 py-4 text-sm text-gray-700">{{ $user->apartment->number ?? '-' }}</td>
                            <td class="px-6 py-4 text-sm text-center space-x-2">
                                ...
                                <form action="{{ route('users.ban', $user->id) }}" method="POST" class="inline">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit" class="px-3 py-1 rounded bg-red-500 text-white hover:bg-red-600 transition">Забанить</button>
                                </form>
                            </td>
                        </tr>
                    @endforeach
