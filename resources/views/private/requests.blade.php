...
@section('content')
    <div class="flex flex-col space-y-4">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('requests.create') }}" class="px-4 py-2 bg-gray-800 text-white rounded hover:bg-gray-700 transition">
                Создать заявку
            </a>
        </div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    @foreach($requests as $request)
                        <tr>
                            ...
                            <td class="px-6 py-4 text-sm text-gray-700">{{ $request->apartment->number }} - {{ $request->apartment->house->name ?? '-' }}</td>
                            ...
                            <td class="px-6 py-4 text-sm text-center space-x-2">
                                <a href="{{ route('requests.show', $request->id) }}" class="px-3 py-1 rounded bg-blue-200 hover:bg-blue-300 transition">Просмотр</a>
